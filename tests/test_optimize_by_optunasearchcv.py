...
from lohrasb.tests_conf import estimators_params_optunasearchcv_clfs, estimators_params_optunasearchcv_regs 
...
def setup_test(estimator, params, score_func, task_type):
    ...
    kwargs = {
        'kwargs': {
            'newoptuna_search_kwargs': {**SEARCH_KWARGS, 'estimator': est, 'param_distributions': params, 'scoring': score_func},
...
for test_type, func in [('clf', f1_score), ('reg', r2_score)]:
    @pytest.mark.parametrize('estimator, params', globals()[f'estimators_params_optunasearchcv_{test_type}s'])
